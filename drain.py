...
def eigrp_drain_config(eigrp_neighbors, delay=100):
	config_list = []
	for neighbor in eigrp_neighbors['TABLE_asn']['ROW_asn']['TABLE_vrf']['ROW_vrf']['TABLE_peer']['ROW_peer']:
		config_dict = dict(interface = 'interface {}'.format(neighbor['peer_ifname']), delay = "delay {}".format(delay))
		config_list.append(config_dict)
	return config_list

def apply_configuration(configuration):
	for config in config_list:
		cli('configure terminal ; {0} ; {1}'.format(config['interface'], config['delay']))
...
#MAIN
configuration = eigrp_drain_config(json.loads(clid("show ip eigrp neighbors")))
